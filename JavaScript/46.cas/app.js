...
// 1. nacin
function objTitBug(movies) {
  const kraj = movies.map((movie) => ({
    title: movie.title,
    budget: movie.budget,
  }));
  return kraj;
}
const moviesWithTitleAndBudget = objTitBug(movies);
console.log(moviesWithTitleAndBudget);
...
function braonIPolozenC(arr) {
  return arr.filter(
    (student) => student.bojaOciju === "braon" && student.polozenC === false
  );
}
const rezultat = braonIPolozenC(arr);
